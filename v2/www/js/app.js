$(document).ready(function () {
		...
		if (phone_number.length == 0) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (status == "success") {
						/*window.FirebasePlugin.verifyPhoneNumber(phone_number, "120", 
							function(_credential) {
								SMSReceive.startWatch(
									function() {
										$(messageBox).html("<img src='img/load-sm.gif' alt='' /> Authenticating number...").hide().fadeIn();
									}, 
									function() {
										alert('Error: Plugin failed to start');
									}
								);
								*/
								/*document.addEventListener('onSMSArrive', function(e) {
									var sms 		= e.data;
									if(sms.address == "+123456" || sms.address == "Phone Code"){										
										*/					
	...
	$("#parcel_list").delegate(".parcel_details", "click", function (event) {
		...
		capture_photo 	= "";
		app_notes 		= "";
		$(".scan_barcode").removeClass("success");
	...
	$(".menu-btn").click(function(){
		if (barcode_data) {
			$(".scan_barcode").addClass("success");
			$(".sign_btn").removeClass("not-active");
			$(".capture_photo").removeClass("not-active");
			$(".enter_notes").removeClass("not-active");
		}else{
			$(".sign_btn").addClass("not-active");
			$(".capture_photo").addClass("not-active");
			$(".enter_notes").addClass("not-active");
		}

		if(customer_sign == ""){
			$(".sign_btn").removeClass("success");
		}else{
			$(".sign_btn").addClass("success");
		}

		if(capture_photo == ""){
			$(".capture_photo").removeClass("success");
		}else{
			$(".capture_photo").addClass("success");
		}

		if(app_notes == ""){
			$(".enter_notes").removeClass("success");
		}else{
			$(".enter_notes").addClass("success");
		}
	});
